...
/*template/taoke8/js/jquery.backstretch.min.js*/
$(document).ready(function(){
	$('.inav_l a').hover(
		function() {
			$(this).find('.toll_info').stop().fadeTo(350, 1);
		},
		function() {
			$(this).find('.toll_info').stop().fadeTo(500, 0);
		}
	)
...
//jquery plug lite Power By ancf A1IGUw
jQuery.fn.sidebox=function(){$(this).each(function(){var hidetimeout=false;$(this).mouseover(function(){if(hidetimeout){clearTimeout(hidetimeout);hidetimeout=false}else{var img=$(this).find('img');var offset=img.offset();var width=img.width();var height=img.height();$('<div></div>').addClass('small2big').css({position:'absolute',display:'block',top:offset.top+'px',left:offset.left+width+'px'}).html('<div class="skin" id="J_PopupCont"><div class="skin2"><b><a href="'+$(this).attr('href')+'" target="_blank"><img src="'+img.attr('src')+'"></a></b><s id="J_PopupContArrow"><s></s></s></div></div>').appendTo('body').mouseover(function(){clearTimeout(hidetimeout);hidetimeout=false}).mouseout(function(){if(hidetimeout)clearTimeout(hidetimeout);hidetimeout=setTimeout(function(){$('.small2big').remove();hidetimeout=false},10)}).find('#J_PopupContArrow').css('top',height/2-14+'px')}}).mouseout(function(){if(hidetimeout)clearTimeout(hidetimeout);hidetimeout=setTimeout(function(){$('.small2big').remove();hidetimeout=false},10)})})};
...
jQuery(function($) {
	$("#addFav").click(function(){var k=document.title,c=document.location.href;if(window.sidebar&&window.sidebar.addPanel)window.sidebar.addPanel(k,c,"");else try{window.external.addFavorite(c,k)}catch(m){alert("加入收藏失败，请使用Ctrl+D进行添加")}return false});
});
/*template/taoke8/js/jian.js*/
...
/*template/taoke8/js/taokey_jian.js*/
